...
4a. Debt check (skip if no `docs/debt.md`): grep the shelf for each required-touchpoint path. A hit is debt in a file you're about to open anyway — the fix is nearly free. Carry hits into the report.
5. Report first: selected bucket, required touchpoints read, current behavior, proposed edits, validation plan, extra touchpoints if needed, and — for any debt-check hit — a fold-in / sibling-bucket / leave-parked call (fold in only if trivial and within this bucket's budget; else propose a sibling bucket). On fold-in, check the row off in `docs/debt.md` as part of the bucket's edits.
